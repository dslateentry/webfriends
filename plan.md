# WebFRIENDS — Plan de Construcción

## Completado (✅)
- Auth Supabase (login/register), perfiles, logout
- 8 vistas SPA con sidebar collapsible
- Layout: `flex:1;margin-left:185px;position:relative` + `padding-left:190px` en vistas no-Home
- iPod centrado en `#app-main` (fuera de desktop-area, `left:50%+translateX(-50%)`, `toggleSidebar` ajusta marginLeft)
- 4 pop-ups Win95 (Tama, Boombox, Chat, TV)
- Avatar Paper Doll Bratz (F/M/X) + Sims 2 bubbles + profile photo
- iPod YouTube (autoplay muteado en restore, con sonido al click)
- Boombox CD + Spotify embed
- Fotolog con Supabase Storage (bucket `photos`) + data-attributes + event delegation
- Fotos compartidas entre usuarios (Supabase Storage + Realtime cambios en vivo)
- Posiciones persistentes de user cards y polaroids
- Calendario, Foodie, Notepad, Notitas, Foryou
- User cards + polaroids en escritorio
- Burbujas iPhone secuenciales
- Filmstrip GIFs infinito
- WebRTC (PeerJS) + screen sharing
- Ephemeral guest links (UUID, TTL 5 min)
- Tamagotchi (decay 40s, evolución 🥚→🐣→🐤→🐔)
- Emoji Drop, Post-its caóticos
- 3 temas (Night/Sky toggle)

## TO-DO
- [ ] Chat en tiempo real (Supabase Realtime)
- [ ] Minijuegos en Arcade
